...
中核となる課題は、厳密な屋内のミリ単位の精度を国土交通省主導の**PLATEAU**データセットの広域地理座標系と一致させることでした。Blender + Bonsaiの抽出ワークフローを自社開発し、モデリングした駅構内データを正しいCityGMLトポロジへと外部出力。地下ホームと地上道路ネットワークを途切れることなく接続しました。
